...
const( my $mmf_basename => "fcgi_spawn_" . rand(100500) . "_scoreboard.lck",
);
...
const(
    my $conf_presets => {
        ...
        'chroot'            => {
            'cgi_dir'  => "/$etc_test/cgi",
            'cmd_args' => [ '-mmf' => "$etc_test/$mmf_basename", ],
        },
    },
);
const(
    my $b_conf => realpath( dirname(__FILE__) . "/../../../../$etc_test" ) );

my %bnames = (
    'etc_test'  => $etc_test,
    'pid_fname' => 'fcgi_spawn.pid',
    'log_fname' => 'fcgi_spawn.log',

    #'sock_name' => "spawner.sock", # TODO: local socket
    'sock_name'      => "127.0.0.1:8888",
    'cmd_args_first' => [
        "-Iblib/lib" => "bin/fcgi_spawn",
        "-mmf"       => "$etc_test/$mmf_basename",
        '-cm',
    ],
...
foreach my $p_name ( keys %bnames ) {
    ...
    my $default;
    if ($ref) {
        $default = subname( "init_$p_name" => sub {$b_default}, );
    }
...
sub kill_proc_dead : Export( :DEFAULT ) {
    ...
    unless ( is_process_dead($pid) ) {
        foreach my $i ( 1 .. $times ) {
            ...
            sleep 1;
            diag(
                "TERM wait RV: $rv " . "kill0: " . kill( 0 => $pid ) . "\n" )
                if $debug;
...
sub spawn_fcgi {
    my $self = shift;
    my $nd = @_ > 0 ? shift : 0;
    my ( $conf_dname, $log_fname, $pid_fname, $sock_name, $user, $group,
        $cmd_args_first )
        = map {
        my $s_name = "get_$_";
        $self->$s_name;
    ...
    if ( $UID == 0 ) { push( @args, "-u" => $user, "-g" => $group, ); }
        ...
        push(
            @args,
            "-u" => scalar getpwuid($UID),
            "-g" => scalar
                getgrgid( shift( @{ [ split( /\s+/, $GID ), ] }, ), )
        );
    }
...
sub rm_files_if_exists {
    ...
    croak( "Open directory: $b_conf", )
        unless opendir my $b_conf_dh => $b_conf;
    ...
    try {
        ...
        if ( defined $addr ) {
            $addr = gethostbyname($addr)
                unless $addr =~ m/^(\d{1,3}\.){3}\d{1,3}$/;
            croak("sockarrd_in $sock_name: $@ $!")
                unless my $struct_addr =
                    sockaddr_in( $port, inet_aton($addr) );
            croak("socket() for $sock_name: $@ $!")
                unless
                socket( my $h, PF_INET, SOCK_STREAM, getprotobyname('tcp') );
