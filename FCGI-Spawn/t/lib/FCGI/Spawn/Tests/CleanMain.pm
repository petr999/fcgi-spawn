...
has('old_content' => (
        qw/is rw   isa ArrayRef   default/ =>
            subname( 'init_old_content' => sub { [] }, ),
    ),
);
has( '+descr' =>
        ( 'default' => 'Cleaning global variables from CGI programs', ), );
has( '+env'       => (qw/is rw/) );
...
sub make_env {
    ...
    my $env      = {
        %$orig_env,
        qw/REQUEST_METHOD POST CONTENT_TYPE/ =>
...
sub remake_content {
    ...
    my $content =
        \( encode_json( { 'old' => $old_content, 'new' => $new_content, } ) );
