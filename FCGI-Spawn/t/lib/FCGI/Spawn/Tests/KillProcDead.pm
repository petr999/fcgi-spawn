...
sub check {
    ...
    if ( defined $pid ) {
        if ($pid) {
            ...
            $rv =
                ok( kill_proc_dead($pid) =>
                    'Killing TERM-ignorant sleeping process' );
            sleep 5;
            $rv &&= is(
                $shared => 123,
                'action is taken after sleep despite TERM sent'
            );
        }
        else {
            bore_proc(3);
    ...
    if ($rv) {
        ...
        if ( defined $pid ) {
            if ($pid) {
                select( undef, undef, undef, 0.025 );
                ok( kill_proc_dead( $pid => 2, ) =>
                        'Killing TERM-ignorant sleeping process', );
                sleep 10;
                is( $shared => 0,
                    'action is not taken after sleep because KILL was sent'
                );
            }
            else {
                bore_proc(7);
