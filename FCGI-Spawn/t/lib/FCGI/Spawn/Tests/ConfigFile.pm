...
sub check {
    ...
    foreach my $sfx ( '' => 'Moose', ) {
        ...
        if ( $rv
            &&= ok( $conf = $class->new() => 'Config file object creation', )
            )
        {
            my $descr = $self->get_descr;
            $descr .= " with $sfx";
            my $config_path =
                realpath "$FindBin::Bin/../t/etc-test/clean_inc_sub_ns";
            $rv = ok( $conf->read_fsp_config_file($config_path) => $descr, );
            ok( $conf->{ 'clean_inc_subnamespace' } ~~
                    [qw/CleanIncSubNs StatsCisns/],
                'Array reading'
            );
