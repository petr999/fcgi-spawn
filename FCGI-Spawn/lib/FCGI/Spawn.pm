...
const $defaults => {
    ...
    'deeds' => {},         # initial deeds is empty

    # CGI.pm is used but no need in its ->_reset_globals with mod_perl
    qw/use_cgi 1/,
...
# Takes     :   constructor properties
# Throws    :   on corresponding acceptor module load fails
...
# Depends	:	on $defaults package lexical variable
...
sub _callout {
	...
	if ( $$self{ 'stats'    } ) { $self->set_state( 'deeds', $_[0] => 1 ); }
    $$self{ callout }( @_ => $fcgi );
...
# Object method
# Gets stats of the files in the hash
    ...
    foreach my $key (@keys_arr) {
        ...
        if ( $value eq $module ) {
            ...
            if ( @$ns > 0 ) {
                ...
                my $rv = eval {
                    delete $INC{ $key };
                    require $module unless defined $$deeds{ $module };
                    1;
                };
                unless ($rv) {
