...
sub check{ 
  ...
  my @tests = (
    ...
    sub{ is( get_fork_rv( sub{ return 3; } ) => 3,
      'Get value from fork()' ); },
    sub{ ok( $util = FCGI::Spawn::TestUtils -> new,
      => 'TestUtils constructor' ); },
    sub{ ok( $timeout = $util->get_timeout
      => 'Get timeout from utils', ); },
    sub{ cmp_ok( $pid = get_fork_pid( sub{ sleep $timeout; }, ),
      '>', 0, 'Get PID from fork()', ); },
