...
sub upload {
    ...
    return map( { Apache::Upload->new( $q, $_, $next, ); }
        keys( %{ $$self{ 'UPLOADS' } } ) )
        if wantarray;
    return Apache::Upload->new( $q, ( keys %{ $$self{ 'UPLOADS' } } )[0],
        $next, );
}
...
sub new {
    ...
    my $class = ref($caller) || $caller;
    return
        bless( { 'CGI' => $q, 'NAME' => $name, 'NEXT' => $next, } => $class,
        );
}
...
my @remotehost =
    qw/REMOTE_HOST REMOTE_NAME REMOTE_NOLOOKUP REMOTE_DOUBLE_REV/;
...
our @EXPORT_OK = (
    @response, @http,   @options, @methods,  @remotehost, @satisfy,
...
sub jar {
    ...
    my $rv = { map { $_ => $cookies->{ $_ }->value; } keys %$cookies };
    return $rv;
}
...
sub header_in {
    my ( $self, $key, $val, ) = @_;
    if ( defined $val ) {
        $$self{ 'HEADERS_IN' }{ ucfirst( lc($key) ) } = $val;
    }
    $$self{ 'HEADERS_IN' }{ $key };
}
...
sub get_remote_host {
    my $self = shift;
    $self->{ 'REMOTE_HOST' } || $self->{ 'REMOTE_ADDR' };
}
...
sub send_http_header {
    ...
    if ( defined $$self{ 'HEADERS_SENT' } ) {
        ...
        foreach my $header ( keys %{ $$self{ 'EHEADERS_OUT' } } ) {
            $self->print(
                $header . ': ' . $$self{ 'EHEADERS_OUT' }{ $header } . "\n" );
...
sub soft_timeout {
    my ( $self => $message, ) = @_;
    $SIG{ 'ALRM' } = subname(
        'soft_timeout_sig_alrm' => sub { $$self{ 'ABORTED' } = $message; }, );
    alarm(120);
}
...
sub new {
    ...
    if ( $sa = getsockname(STDIN) ) {
        ( $conf{ 'LOCAL_PORT' } => $conf{ 'LOCAL_ADDR' }, ) =
            sockaddr_in($sa);
    ...
    if ( $sa = getpeername(STDIN) ) {
        ( $conf{ 'REMOTE_PORT' } => $conf{ 'REMOTE_ADDR' }, ) =
            sockaddr_in($sa);
    ...
    my @modules;
    my $vars     = Apache::Table->new();
    my $handlers = {};
    my ( $docroot => $admin, );
    my ( $aliases => $requires, ) = ( [] => [], );
    my ( $ctx, $loc, $rest, );
    my $addContext = subname(
        'add_context' => sub {
            return unless $ctx;
            push( @modules => map { join( " " => @{ $_ }, ); }
                    $ctx->cmd_config_array( 'PerlModule', ), );
            ...
            if ( $ctx->cmd_config_array('PerlHandler') ) {
                ( $$handlers{ 'PerlHandler' } ) =
                    map { @{ $_ } } $ctx->cmd_config_array( 'PerlHandler', );
            }
            if ( $ctx->cmd_config_array( 'DocumentRoot', ) ) {
                ($docroot) =
                    map { @{ $_ } } $ctx->cmd_config_array( 'DocumentRoot', );
            }
            if ( $ctx->cmd_config_array( 'ServerAdmin', ) ) {
                ($admin) =
                    map { @{ $_ } } $ctx->cmd_config_array('ServerAdmin');
            }
            if ( $ctx->cmd_config_array( 'ServerAlias', ) ) {
                $aliases =
                    [ map { @{ $_ } } $ctx->cmd_config_array('ServerAlias') ];
            }
            if ( $ctx->cmd_config_array( 'requires', ) ) {
                $requires =
                    [ map { @{ $_ } } $ctx->cmd_config_array('requires') ];
            }
        },
    );
    $conf{ 'VIRTUAL' } = 0;
    ...
    if ($with_config_file) {
        unless ( defined $conf{ 'dir_conf' } ) {
            $conf{ 'dir_conf' } = '.htaccess';
        }
        if ( defined $conf{ 'handler_cgi' } ) {
            die('virtual_root missing')
                unless defined $conf{ 'virtual_root' };
            $ENV{ 'PATH_TRANSLATED' } =
                $conf{ 'virtual_root' } . $ENV{ 'PATH_INFO' };
            $ENV{ 'PATH_INFO' } =
                $conf{ 'handler_cgi' } . $ENV{ 'PATH_INFO' };
        }
